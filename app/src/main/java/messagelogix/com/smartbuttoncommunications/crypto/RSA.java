...
/**
 * Created by Vahid
 * This class is related to RSA encryption and decryption in our app
 */
public class RSA {
    ...
    public static byte[] encrypt(Key publicKey, byte[] toBeCiphered) {

        try {
            //"RSA/ECB/PKCS1Padding
            Cipher rsaCipher = Cipher.getInstance("RSA/None/OAEPWithSHA1AndMGF1Padding", "SC");
            rsaCipher.init(Cipher.ENCRYPT_MODE, publicKey);
            return rsaCipher.doFinal(toBeCiphered);
        } catch (Exception e) {
    ...
    public static String encryptToBase64(Key publicKey, String toBeCiphered) {

        byte[] cipheredText = RSA.encrypt(publicKey, toBeCiphered.getBytes());
    ...
    public static String decryptFromBase64(Key key, String cipheredText) {

        byte[] afterDecrypting = RSA.decrypt(key, Base64.decode(cipheredText, Base64.DEFAULT));
